...
INSERT INTO T_BALANCE(account_id, tenant_id, amount, currency)
    SELECT account_id, tenant_id, sum(ABS(net)), currency FROM t_transaction WHERE type=1 AND status=1 GROUP BY account_id, tenant_id, currency;
...
INSERT INTO T_BALANCE(account_id, tenant_id, amount, currency)
    SELECT recipient_id, tenant_id, sum(ABS(net)), currency
        FROM T_TRANSACTION
