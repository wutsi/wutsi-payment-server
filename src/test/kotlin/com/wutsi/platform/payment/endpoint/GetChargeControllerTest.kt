...
public class GetChargeControllerTest : AbstractSecuredController() {
    ...
    public fun `get charge`() {
        ...
        assertEquals(PaymentMethodProvider.PAYMENT_METHOD_PROVIDER_MTN.shortName, charge.paymentMethodProvider)
        assertEquals(PaymentMethodType.PAYMENT_METHOD_TYPE_MOBILE_PAYMENT.shortName, charge.paymentMethodType)
        assertEquals("urn:order:1111", charge.externalId)
        assertEquals(10000.0, charge.amount)
        assertEquals("XAF", charge.currency)
        assertEquals(Status.STATUS_FAILED.name, charge.status)
